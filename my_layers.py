...
class MyMaxPool(Layer):
    def __init__(self, axis, smask, **kwargs):
    ...
    def compute_mask(self, x, mask=None):
        ...
        # print("Max_compute_mask:mask", mask)
        return None

    def call(self, x, mask=None):

        #mask_ = K.ones((x.shape.as_list()[0], x.shape.as_list()[1]))
        #mask_reverse = mask^mask_ # 
        # if mask is not None:
        mask1 = K.repeat(self.smask, x.shape[-1])
        mask1 = tf.transpose(mask1, [0,2,1])
        mask1 = K.cast(mask1, K.floatx())
        x = x * mask1
        # print('x',x)
        # print("max_call:mask", mask1)
        # print(K.sum(x, axis=self.axis) / K.sum(mask, axis=self.axis))
        mask_ = K.ones_like(mask1)#(x.shape.as_list()[0], x.shape.as_list()[1]))
        #mask_ = K.cast(mask_, dtype=bool)
        mask_reverse = mask_ - mask1 # 
        #mask_reverse = K.repeat(mask_reverse, x.shape[-1])
        #mask_reverse = tf.transpose(mask_reverse, [0,2,1])
        #mask_reverse = K.cast(mask_reverse, K.floatx())
        mask_reverse = mask_reverse * (-float('inf'))
        x = x + mask_reverse
...
class MyMeanPool(Layer):
    ...
    def compute_mask(self, x, mask=None):
        # need not to pass the mask to next layers
        # print("Mean_compute_mask:mask", mask)
        pass
        # return None
    ...
    def call(self, x, mask=None):
        ...
        x = x * mask1
        # print('x',x)
        # print("mean_call:smask", self.smask)
        # print(K.sum(x, axis=self.axis) / K.sum(mask, axis=self.axis))
        return K.sum(x, axis=self.axis) / (K.sum(mask1, axis=self.axis) + K.epsilon())
        # else:
        #     print("mean_call:mask None!!!", mask)
